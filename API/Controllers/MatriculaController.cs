...
public class MatriculaController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<MatriculaDto>>> GetPaginacion([FromQuery] Params rolParams)
    {
        var entidad = await unitofwork.Matriculas.GetAllAsync(rolParams.PageIndex, rolParams.PageSize, rolParams.Search);
        var listEntidad = mapper.Map<List<MatriculaDto>>(entidad.registros);
        return new Pager<MatriculaDto>(listEntidad, entidad.totalRegistros, rolParams.PageIndex, rolParams.PageSize, rolParams.Search);
    }
